...
def extract_sand_poly(metadata, settings, isUkulhasMed):
    ...
    for satname in metadata.keys():
        ...
        for i in range(len(filenames)):
            ...
            im_classif, im_labels = SDS_shoreline.classify_image_NN(im_ms, cloud_mask,min_beach_area_pixels, clf)
            ...
            if 'reference_shoreline' in settings.keys():
                ...
                ref_sl_pix = SDS_tools.convert_world2pix(SDS_tools.convert_epsg(ref_sl, settings['output_epsg'], image_epsg), georef)
                ##ref_sl_pix = SDS_tools.convert_world2pix(SDS_tools.convert_epsg(ref_sl, settings['output_epsg'],image_epsg)[:,:-1], georef)  
            ...
            try:
                if sum(sum(im_labels[:,:,0])) == 0 :
                    ...
                    contours_mndwi, t_mndwi = SDS_shoreline.find_wl_contours1(im_mndwi, cloud_mask, im_ref_buffer)
                else:
                    # use classification to refine threshold and extract sand/water interface
                    contours_mndwi, t_mndwi = SDS_shoreline.find_wl_contours2(im_ms, im_labels, cloud_mask, im_ref_buffer)

            except Exception as error:
                print('Could not map shoreline for this image: ' + filenames[i], error)
                continue

            # process water contours into shorelines
            cloud_mask_adv = np.logical_xor(cloud_mask, im_nodata) 

            #contours_mwi,contours_wi
            shoreline = SDS_shoreline.process_shoreline(contours_mndwi, cloud_mask_adv, im_nodata, georef, image_epsg, settings)
    ...
    gdf = SDS_tools.output_to_gdf(output,'points')  ##    geomtype: str'lines' for LineString and 'points' for Multipoint geometry 
    # set projection
